...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		if (i == 0) {
			...
			if (i * PGSIZE + KERNBASE < (size_t)nextfree) {
...
{
	pte_t *pte;
	for (uintptr_t va_i = va; va_i < va + size; ++va_i) {
		(*pgdir_walk(pgdir, (void*)va, 1)) |= ((perm|PTE_P) & 0xFFF);
		(*pgdir_walk(pgdir, (void*)va, 1)) |= (~0xFFF & ((pa+va_i-va) << 12));
	}
}
